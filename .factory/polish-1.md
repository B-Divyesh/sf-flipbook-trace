...
| --- | --- | --- |
| F-1-1 | Added the compact twelve-frame `#demo-strip` immediately below the demo heading. | `@claim:demo-ready`; `the one-click demo shows a sample frame…`; `test-results/polish-demo-390.png`, `test-results/polish-demo-1440.png`; live cold check after deploy. |
| F-1-2 | Added designed `public/404.html` and SWA `responseOverrides.404` rewrite. | `the deployment configuration…`; `the designed static 404 artifact is served with HTTP 404`; live cold check after deploy. |
...
| F-1-10 | Checkout claim follows the production redirect and asserts the Dodo session’s product, USD 9.00, and one-time wording. | `@claim:studio-purchase`; live checkout response check after deploy. |
| F-1-11 | Removed unsupported format names; copy says “Choose a video this browser can play.” | copy audit; live cold check after deploy. |
...
| F-1-15 | Route metadata now updates title, description, canonical, Open Graph URL/title/description, and Twitter values. | `updates Open Graph and Twitter route metadata`; live route check after deploy. |
| F-1-16 | Standardized the user input as “video” and duration as “selected section” across UI and README. | copy audit; `rg` terminology check; live cold check after deploy. |
...
- The deployed cold-route check is recorded in the handoff after the production host finishes the static deployment.
