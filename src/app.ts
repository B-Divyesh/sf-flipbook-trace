...
const DEMO_SOURCE_WIDTH = 120;
const DEMO_SOURCE_HEIGHT = 75;
...
async function loadDemoFrames(count = 12): Promise<void> {
  ...
  // The first-screen sample needs its finished tracing frames, not the hidden
  // colour source frames. Draw only that useful result here. Source canvases
  // are built on the first adjustment or export, so their work cannot join a
  // cold mobile startup task.
  await yieldToBrowser();
  ...
  if (settings.mode !== 'edges' || settings.threshold !== defaultSettings.threshold || settings.onion) schedulePreviewRender();
}

async function ensureDemoSourceFrames(generation: number, count = outputFrames.length): Promise<boolean> {
  if (!isDemo || sourceFrames.length) return true;
  const { drawDemoFrame } = await loadFrameProcessor();
  const sources: HTMLCanvasElement[] = [];
  // Keep each source drawing in its own task. This runs only after a person
  // asks for a different trace treatment or an export.
  await yieldToBrowser();
  for (let index = 0; index < count; index += 1) {
    if (generation !== previewGeneration || !isDemo) return false;
    const canvas = document.createElement('canvas');
    canvas.width = DEMO_SOURCE_WIDTH;
    canvas.height = DEMO_SOURCE_HEIGHT;
...
function schedulePreviewRender(delay = 0): void {
  if (!sourceFrames.length && !outputFrames.length) return;
  cancelPreviewRender();
  setExportButtonsDisabled(true);
  const generation = previewGeneration;
...
async function rebuildOutputFrames(generation: number): Promise<void> {
  if (isDemo && !await ensureDemoSourceFrames(generation)) return;
  if (generation !== previewGeneration) return;
...
function buildExportFrameStream(onFrame?: (index: number, total: number) => void): AsyncGenerator<HTMLCanvasElement> {
  ...
  return (async function* exportFrames() {
    if (isDemo && !await ensureDemoSourceFrames(previewGeneration)) return;
    const sources = [...sourceFrames];
    const { applyTraceFilter } = await loadFrameProcessor();
    for (let index = 0; index < sources.length; index += 1) {
