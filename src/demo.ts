...
function drawFrames(): void {
  ...
  for (let index = 0; index < 12; index += 1) {
    ...
    drawSampleTrace(canvas, index);
    canvas.setAttribute('role', 'img');
    canvas.setAttribute('aria-label', `Tracing frame ${index + 1} of 12`);
...
function boot(): void {
  app.innerHTML = `${header()}<aside class="demo-banner" aria-label="Demo mode"><strong>Demo — sample data, nothing is saved</strong><div><button id="reset-demo" type="button">Reset demo</button><a href="/">Start for real</a></div></aside><main id="main" class="demo-main"><section id="demo-intro" class="demo-intro"><p class="eyebrow">Paper-bird sample</p><h1 tabindex="-1">Trace a paper bird in twelve frames</h1><p>The sample is built into the app and works without a network.</p><div class="demo-peek" aria-label="Twelve sample tracing frames"><div id="demo-strip" class="demo-strip"></div><p>12 ready frames · frame controls are loading</p></div></section><section class="workspace" aria-labelledby="workspace-heading"><div class="section-kicker">01 / Prepare</div><div class="workspace-heading-row"><div><h2 id="workspace-heading">Make the tracing frames</h2><p>The paper-bird sample is ready. Frame controls will appear below.</p></div><output id="work-status" class="status-stamp" aria-live="polite">Preparing sample frames…</output></div><div class="preview-zone"><div id="frame-strip" class="frame-strip" aria-label="Tracing frame preview"></div></div></section></main><div id="route-status" class="sr-only" aria-live="polite"></div>${footer()}`;
  drawFrames();
  const status = document.querySelector<HTMLOutputElement>('#work-status');
  if (status) status.value = '12 frames ready';
  document.querySelector<HTMLButtonElement>('#reset-demo')?.addEventListener('click', drawFrames);
